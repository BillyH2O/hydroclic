...
export function Footer({ className, ...props }: StickyFooterProps) {
	return (
		<footer
			className={cn('relative bg-white lg:h-[420px] h-[820px] w-full flex border-t border-zinc-200', className)}
							...
							{footerData.linkGroups.map((group, index) => (
								...
								>
									<div className="mb-10 md:mb-0">
										<h3 className="text-sm uppercase text-zinc-900 font-semibold">{group.label}</h3>
										<ul className="text-zinc-600 mt-4 space-y-2  text-sm md:text-xs lg:text-sm">
											{group.links.map((link) => (
												<li key={link.title}>
													<a
														href={link.href}
														className="hover:text-blue-600 inline-flex items-center transition-all duration-300 hover:translate-x-1 cursor-pointer"
													>
														{link.title}
													</a>
												</li>
