...
const OPEN_EVENT = 'hydroclic:open-cookie-preferences'
...
export function CookieBanner() {
  ...
  const decide = (nextAnalytics: boolean) => {
    applyConsent(persistConsent(nextAnalytics))
  }
    ...
    >
      <div className="mx-auto max-w-3xl rounded-2xl border border-zinc-200 bg-white p-5 shadow-2xl sm:p-6">
        ...
        <p id="cookie-banner-desc" className="mt-2 text-sm leading-relaxed text-gray-600">
          Nous utilisons des cookies strictement nécessaires au compte, au panier et au paiement.
          Aucun cookie de mesure d’audience ou publicitaire n’est déposé aujourd’hui. Vous pouvez
          refuser les cookies optionnels aussi facilement que les accepter.{' '}
          ...
          <label className="mt-4 flex items-start gap-3 rounded-lg border border-zinc-200 bg-zinc-50 p-3 text-sm text-gray-700">
            ...
            <span>
              ...
              <span className="block text-gray-500">
                Inactif : aucun traceur analytique n’est actuellement chargé sur le site.
              </span>
            </span>
          </label>
        ) : null}
        ...
        <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-end">
          ...
          {customize ? (
            <button
              ...
              className="h-10 rounded-md bg-primary px-4 text-sm font-medium text-white hover:bg-primary/90"
            >
